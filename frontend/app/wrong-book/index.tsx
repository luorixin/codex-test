...
export default function WrongBookScreen() {
  ...
  const subjectIdParam = typeof params.subjectId === 'string' ? params.subjectId : undefined;
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      load(subjectIdParam).catch((error) => {
        if (!cancelled) {
          setErrorMessage(
          ...
          <View style={styles.filterWrap}>
            {screenData.subjects.map((subject) => (
              <PrimaryButtonChip
                ...
                onPress={() => handleSelectSubject(subject.subjectId)}
              />
            ))}
          </View>
        ) : (
