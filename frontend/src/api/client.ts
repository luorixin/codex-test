...
import {
  ...
  logoutUnauthorized,
  refreshSession,
} from '@/src/api/authRuntime';
import { getBackendBaseUrl } from '@/src/config/runtime';
...
function applyAuthorizationHeader(
  ...
  if (options?.overwrite || !headers.has('Authorization')) {
...
async function tryRefreshAndRetryRequest(error: unknown) {
  ...
  if (requestConfig.skipAuthRefresh || requestConfig._retry) {
    if (!requestConfig.skipUnauthorizedLogout) {
  ...
  return apiClient.request(
    applyAuthorizationHeader(requestConfig, refreshedAccessToken, {
      overwrite: true,
    }),
  );
}
